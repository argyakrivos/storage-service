...
object Status extends Ordering[Status] {
  ...
  def apply(progress: Progress): Status = {
    ...
    if (progress.isDone)
      ...
      Status(now + etaClicks, !available, (written/size)*100)
    }
  }
}
...
class InMemoryRepo extends StorageProviderRepo {
  ...
  override def storeProgress(jobId: JobId, progress: Progress) = Future(repo.put _)
  override def getProgress(jobId: JobId) = Future(repo.get(jobId))
