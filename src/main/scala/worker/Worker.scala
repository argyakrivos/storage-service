...
case class QuarterMasterStorageWorker(swConfig:StorageWorkerConfig ) extends StorageService {
  ...
  def isStatusFailure(t:(DelegateType, Status)): Boolean = (t._2 == Status.neverStatus)

  def cleanUpIfFailed(assetToken:AssetToken, label:Int)(resultSet:Set[(DelegateType, Status)]): Future[Set[(DelegateType, Status)]] = {
    resultSet.exists(isStatusFailure) match {
      case true   => cleanUp(assetToken, label )
      case false  => Future{resultSet}
    }
  }
