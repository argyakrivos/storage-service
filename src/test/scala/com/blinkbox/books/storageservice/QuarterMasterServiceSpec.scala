...
//  }
  ...
  it should "save an artifact" in {
    ...
    Post("/resources", formData) ~> routes ~> check {
      ...
      assert(header("location").nonEmpty)
      header("Location") match {
        case Some(RawHeader("Location", storageHeader)) => storageHeader should fullyMatch regex s"http://.+/resources/bbbmap:${label}:.+"
        case _ => throw new NoSuchElementException("Headers did not contain the correct locaiton header.")
      }
    }
  }
}
